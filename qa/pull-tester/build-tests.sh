...
if [ -d "$OUT_DIR" -a -w "$OUT_DIR" ]; then
  set +e
  # Windows:
  cp /root/cryply/win32-build/src/bitcoind.exe $OUT_DIR/bitcoind.exe
  cp /root/cryply/win32-build/src/test/test_bitcoin.exe $OUT_DIR/test_bitcoin.exe
  cp /root/cryply/win32-build/src/qt/bitcoind-qt.exe $OUT_DIR/bitcoin-qt.exe
  # Linux:
  cp /root/cryply/linux-build/src/bitcoind $OUT_DIR/bitcoind
  cp /root/cryply/linux-build/src/test/test_bitcoin $OUT_DIR/test_bitcoin
  cp /root/cryply/linux-build/src/qt/bitcoind-qt $OUT_DIR/bitcoin-qt
  set -e
fi

# Run unit tests and blockchain-tester on Linux:
cd /root/cryply/linux-build
make check

# Run RPC integration test on Linux:
/root/cryply/qa/rpc-tests/wallet.sh /root/cryply/linux-build/src
/root/cryply/qa/rpc-tests/listtransactions.py --srcdir /root/cryply/linux-build/src
# Clean up cache/ directory that the python regression tests create
rm -rf cache

if [ $RUN_EXPENSIVE_TESTS = 1 ]; then
  # Run unit tests and blockchain-tester on Windows:
  cd /root/cryply/win32-build
  make check
fi
