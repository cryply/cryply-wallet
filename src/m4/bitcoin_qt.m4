...
AC_DEFUN([_BITCOIN_QT_FIND_LIBS_WITH_PKGCONFIG],[
  ...
  if test x$auto_priority_version == x; then
    ...
    BITCOIN_QT_CHECK([
      ...
      dnl XXX TODO check if qt is compiled statically
      dnl on mingw pkg-config forgets platforms/libqwindows
      if test x$TARGET_OS == xwindows && test x$have_qt == xyes && test x$bitcoin_qt_got_major_vers == x5; then
        dnl Pretend to be a cmake
        qt_path="$($PKG_CONFIG --variable=prefix Qt5Core 2>/dev/null)"
        qt_platforms=$qt_path/plugins/platforms
        qt_qwindows_prl=$qt_platforms/qwindows.prl
	if test x$qt_path != x && test -f $qt_qwindows_prl; then
          QT_PLATFORM_LIBS=$(grep _PRL_LIBS $qt_qwindows_prl | sed 's|QMAKE_PRL_LIBS.*=||')
          QT_PLATFORM_LIBS=$(echo $QT_PLATFORM_LIBS | sed "s|\$\$.QT_INSTALL_LIBS.|$qt_path/lib|g")
          QT_LIBS="-lqwindows -L$qt_platforms $QT_PLATFORM_LIBS $QT_LIBS"
          QT_CFLAGS="$QT_CFLAGS -DQT_STATICPLUGIN"
        fi
      fi
